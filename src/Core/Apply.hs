...
executePlanWithoutBackup context plan = do
  ...
  case applyResult of
    ...
    Right (applied, failed, skipped) -> return $ ApplyResult
      { success = failed == 0
      , appliedSteps = applied
      , failedSteps = failed
      , skippedSteps = skipped
...
executePlanSteps context steps = do
  ...
  if null failures
    ...
    else if continueOnError options
      then return $ Right (applied, failed, skipped)
...
createErrorResult context errors = ApplyResult
  { success = False
  , appliedSteps = 0
