...
manifestEntryToFileDiff diffType (FileEntry fileEntry) =
    Just $
        FileDiff
            ...
            (path fileEntry)
            Nothing
            (Just $ Mycfg.State.Manifest.checksum fileEntry)
            Nothing
            (Just $ Mycfg.State.Manifest.size fileEntry)
            Nothing
            (Just $ Mycfg.State.Manifest.modifiedTime fileEntry)
            (operation fileEntry)
        ...
        then
            Just $
                FileDiff
                    ...
                    (sourcePath newEntry)
                    (path newEntry)
                    (Just $ Mycfg.State.Manifest.checksum oldEntry)
                    (Just $ Mycfg.State.Manifest.checksum newEntry)
                    (Just $ Mycfg.State.Manifest.size oldEntry)
                    (Just $ Mycfg.State.Manifest.size newEntry)
                    (Just $ Mycfg.State.Manifest.modifiedTime oldEntry)
                    (Just $ Mycfg.State.Manifest.modifiedTime newEntry)
                    (operation newEntry)
        else
            Just $
                FileDiff
