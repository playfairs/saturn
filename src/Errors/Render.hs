...
renderParseError err = case err of
    InvalidToml path msg ->
        color Red (bold "Parse Error:") <+> "Invalid TOML in" <+> pretty (show path)
            <> line
    ...
    InvalidFieldType path field expected ->
        color Red (bold "Parse Error:") <+> "Invalid field type for" <+> color Yellow (dquotes (pretty field)) <+> "in" <+> pretty (show path)
            <> line
            <> indent 2 ("Expected:" <+> color Green (pretty expected))
    CircularImports modules ->
        color Red (bold "Parse Error:") <+> "Circular imports detected"
            <> line
...
renderValidationError err = case err of
    ...
    ConflictingFiles files ->
        color Red (bold "Validation Error:") <+> "Conflicting files:"
            <> line
    ...
    CyclicDependencies deps ->
        color Red (bold "Validation Error:") <+> "Cyclic dependencies:"
            <> line
...
renderModuleError err = case err of
    ...
    DependencyResolutionFailed deps ->
        color Red (bold "Module Error:") <+> "Dependency resolution failed for:"
            <> line
            <> indent 2 (vsep (map (color Yellow . pretty) deps))
