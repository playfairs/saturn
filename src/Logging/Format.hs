...
formatEventBody event = case event of
  ...
  ConfigLoaded{..} -> 
    "Config loaded:" <+> pretty configPath <+> 
    if null modules then "" else parens ("modules:" <+> hsep (map pretty modules))
  
  ConfigParseFailed{..} -> 
    "Config parse failed:" <+> pretty configPath <> line <>
  ...
  ModuleLoaded{..} -> 
    "Module loaded:" <+> pretty moduleName <+>
    if null dependencies 
      then "" 
      else parens ("deps:" <+> hsep (map pretty dependencies))
  
  ModuleLoadFailed{..} -> 
    "Module load failed:" <+> pretty moduleName <> line <>
    indent 2 (color Red (dquotes (pretty error)))
  ...
  ApplyFailed{..} -> 
    "Apply failed:" <+> color Red (dquotes (pretty error)) <+> 
