...
import System.Directory (doesFileExist, doesDirectoryExist, createDirectoryIfMissing, removeDirectoryRecursive, getDirectoryContents)
...
deleteGeneration store genId = do
  currentGen <- getCurrentGeneration store
  case currentGen of
    Right (Just current) | generationId (metadata current) == genId -> 
...
getGenerationStatistics store = do
  ...
  case genList of
    Left err -> return $ Left err
    Right generations -> do
      let statusCounts = Map.fromListWith (+) 
